...
type SKeySpriteSheetProps = {
  pressedKeys: string[];
};

function SKeySpriteSheet({ pressedKeys }: SKeySpriteSheetProps) {
  const { deltaTime } = useGameStore(useShallow((state) => state));

  const tiles = useRef({
    keyUp: [
      { x: -80, y: -64, active: true, width: 16, height: 16 },
      { x: -80, y: -401, active: false, width: 16, height: 15 },
    ],
    keyDown: [
      { x: -80, y: -177, active: false, width: 16, height: 15 },
      { x: -80, y: -290, active: false, width: 16, height: 14 },
    ],
  });
...
export default SKeySpriteSheet;
